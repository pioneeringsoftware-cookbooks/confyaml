...
node['confyaml']['roles'].each do |key, value|
  value ||= {}
  value = {'node_keys' => value} if value.is_a?(String)
  path = value['path'] || key
  node_keys = value['node_keys'] || 'ipaddress'
  path = File.join(value['root'] || node['confyaml']['root'], path)
  node_keys = node_keys.split('/') if node_keys.is_a?(String)
  role = value['role'] || File.basename(path, File.extname(path))
  nodes = search(:node, "role:#{role} AND chef_environment:#{node.chef_environment}") || []
  nodes << node if node['roles'].include?(role) && !nodes.include?(node)
  attributes = nodes.collect { |node| node_keys.inject(node) { |node, node_key| node[node_key] } }
